...
7. Total que hubo que empezar con la limpieza. En este equipo se debate y argumenta todo. Entre personas con un cierto síndrome de diógenes y otras con un enfoque demasiado minimalista llegamos a un equilibrio practico y limpio eliminando estas columnas. (Todas nuestras razones y código lo podéis encontrar en: ENLACE A GIT) Pero resumiendo: con demasiados nulos, duplicados, etc fuera. 
LUZMA - INCISO SOBRE DATOS SOCIODEMOGRÁFICOS
8. Con un nuevo CSV limpio estábamos preparadas para poder saber un poco más de ABC coorporation. Nos interesó la distrinbución de sus empleadops por género (sorpresa, mayoría tíos), la edad media de los empleados es de 35 - 45 años, con un 37% seguidos por los de 25-35 años, con 34.38%
9. Siendo la mayoría jóvenes no ha sido extraño comprobar que para la mayoría era la primera empresa en la que trabajan. Es también interesante ver la distribución de los roles dentro de la empresa, donde podemos comprobar que los puestos con más número de empleados son Ejecutivo de Ventas y Científico de Investigación.
10. También nos preguntamos sobre el nivel de estudiso de nuestros ¿futuros? compañeros de trabajo.
11. Podíamos seguir curioseando pero vamos a centrarnos el problema de ABC: ¿Al rotación si o no?
12. Spoiler: Sí, es bastante alto. Dentro de los datos analizados, rondando el 20% de los empleados ya no están en ABC Corporation.
NEREA - FASE 3 - BBDD
13. Procedimos a insertar los datos, pero se nos presentó aquí el primer reto. Tuvimos problemas a la hora de coger cada elemento del DataFrame y creamos una función para poder utilizarlos. Esta función pasa a tupla y a int los elementos para poder insertarlos correctamente.
14. Posteriormente, concluimos un primer diseño de la base de datos como el que podéis ver en la diapostiva. Pero a pesar de que tuvieramos de manera correcta el formato de los datos, nos pasamos unas cuantas horas intentando averiguar porque no nos cargaba los datos de la tabla combination, nos dimos cuenta de que este diseño era insostenible. Es decir, la tabla de unión de las claves primarias no se rellena automáticamente.
15. Por lo que, tras horas de pensar, tuvimos que rediseñar el esquema, eliminando la tabla combination, y utilizando la primary key de la tabla principal como foreign key de todas las demás, y con ésto, introducción de datos finalizada correctamente. Ya nos permitía hacer todo tipo de consultas.
NEREA -- FASE 4 -- VISUALIZACION 
16. Ya con los datos cocinados y servidos, pasamos a probar el plato planteando nuestra hipótesis: 
...
17. Para contestar a esta preggunta recordemos los estadísticos principales:
- el conjunto de datos totales, unos 1501 empleados, cerca del 20% ya no estaban en la empresa.
- el 60% de los datos valorados son de empleados varones
